  ...
  const msg = {
    ...
    hu_HU: {
      ...
      options: {
        ...
        losses:
          "KÃ¼ldjÃ¶n tÃ¡madÃ¡st olyan falvakba ahol rÃ©szleges vesztesÃ©ggel jÃ¡rhat a tÃ¡madÃ¡s:",
        maxloot:
...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const bindEventHandlers = function () {
    ...
    $(document)
      ...
      .on("keydown", (event) => {
        const key =
          event.key || String.fromCharCode(event.keyCode || event.which);
    ...
    return $.when(buildGroupSelect(options.optionGroup)).then((groupSelect) => {
      return `<style>#popup_box_FarmGod{text-align:center;width:550px;}</style>
                ...
                <div style="width:90%;margin:auto;background: url(\'graphic/index/main_bg.jpg\') 100% 0% #E3D5B3;border: 1px solid #7D510F;border-collapse: separate !important;border-spacing: 0px !important;"><table class="vis" style="width:100%;text-align:left;font-size:11px;">
                  ...
                  <tr><td>${
                    t.options.distance
                  }</td><td><input type="text" size="5" class="optionDistance" value="${
                    options.optionDistance
                  }"></td></tr>
                  <tr><td>${
                    t.options.time
                  }</td><td><input type="text" size="5" class="optionTime" value="${
                    options.optionTime
                  }"></td></tr>
                  <tr><td>${
                    t.options.wallA
                  }</td><td><input type="text" size="5" class="optionWallA" value="${
                    options.optionWallA ?? 0
                  }"></td></tr>
                  <tr><td>${
                    t.options.wallB
                  }</td><td><input type="text" size="5" class="optionWallB" value="${
                    options.optionWallB ?? 0
                  }"></td></tr>
                  <tr><td>${
                    t.options.losses
                  }</td><td><input type="checkbox" class="optionLosses" ${
                    options.optionLosses ? "checked" : ""
                  }></td></tr>
                  <tr><td>${
                    t.options.maxloot
                  }</td><td><input type="checkbox" class="optionMaxloot" ${
                  ...
                  ${
                    game_data.market == "nl"
                      ? `<tr><td>${
                          t.options.newbarbs
                        }</td><td><input type="checkbox" class="optionNewbarbs" ${
                          options.optionNewbarbs ? "checked" : ""
                        }></td></tr>`
                      : ""
                  }
                </table></div><br><input type="button" class="btn optionButton" value="${
                  t.options.button
                }"></div>`;
    });
  };
      ...
      for (let prop in plan) {
        ...
        plan[prop].forEach((val, i) => {
          html += `<tr class="farmRow row_${i % 2 == 0 ? "a" : "b"}">
                    ...
                    }info_village&id=${val.origin.id}">${val.origin.name} (${
                      val.origin.coord
                    })</a></td>
                    <td style="text-align:center;"><a href="${
                      game_data.link_base_pure
                    }info_village&id=${val.target.id}">${
                      val.target.coord
                    }</a></td>
                    ...
                    <td style="text-align:center;"><a href="#" data-origin="${
                      val.origin.id
                    }" data-target="${val.target.id}" data-template="${
                      val.template.id
  ...
  const getData = function (
        ...
        .map((i, el) => {
          ...
          if (coord) {
            ...
            return data.commands[coord].push(
              Math.round(
                lib.timestampFromString($el.find("td").eq(2).text().trim()) /
                  1000,
              ),
            );
          }
    ...
    const enrichWallLevels = () => {
      ...
      const maxDist =
        typeof optionDistance === "number" ? optionDistance : null;
